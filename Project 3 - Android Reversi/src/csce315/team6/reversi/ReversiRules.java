...
public class ReversiRules {
	...
	//*****************************************************************************************
			//****** Below is the code for the Reversi Game Engine ******
			static class Tuple { 
				public final int x; 
				public final int y; 
			...
			static class reversi {
				...
				// Piece objects
				public final static Piece white = Piece.WHITE;
				public final static Piece black = Piece.BLACK;
				public final static Piece empty = Piece.EMPTY;
				public final static Piece valid = Piece.VALID;
				public static Piece player1 = Piece.WHITE;
				public static Piece player2 = Piece.BLACK;
				...
				public static boolean gameOver = false; // true when game is finished
				public static boolean localGame = false; // true for local game, false for AI based game
				private static boolean player1Turn = true; // true when its player 1's turn (or black's turn)
				...
				public static int currentMove; // stores the 3rd dimension value to allow a player to undo and redo moves
				...
				// Gameboard graphics help function ********* 
				private static boolean numPiecesFlippedWasRun = false;
				// ******************************************
				...
				public static void printBoard(int three) {
					if (displayOn) {
						// display current state of board
						System.out.println("   a   b   c   d   e   f   g   h");
						System.out.println(" +---+---+---+---+---+---+---+---+");
						for(int i=0; i<8; i++) {
							System.out.print((i+1) + "| ");
							System.out.print(board[0][i][three]);
							System.out.print(" | ");
							System.out.print(board[1][i][three]);
							System.out.print(" | ");
							System.out.print(board[2][i][three]);
							System.out.print(" | ");
							System.out.print(board[3][i][three]);
							System.out.print(" | ");
							System.out.print(board[4][i][three]);
							System.out.print(" | ");
							System.out.print(board[5][i][three]);
							System.out.print(" | ");
							System.out.print(board[6][i][three]);
							System.out.print(" | ");
							System.out.print(board[7][i][three]);
							if (i == 2) {
								System.out.println(" |  SCORE");
							} else if (i == 3) {
								System.out.println(" | White: " + whiteCount);
					...
					for(int i=0; i<8; i++) {
						for(int j=0; j<8; j++){
							if (isValidMove(i, j, player, true)) {
								...
								// Gameboard Update **************
								Gameboard.getInstance().addValidPiece(i, j);
								// *******************************
							}
						}
					}
				...
				public static void printMenu() {
					...
					System.out.println(" *Enter \"menu\" to show this menu."						);
					...
					for(int i=0; i<8; i++) {
						for(int j=0; j<8; j++){
							if (board[i][j][currentMove] != Piece.EMPTY) {
								...
								board[i][j][currentMove+1] = Piece.EMPTY;
							}
						}
					}
				}
				...
				public static void undo(){
					if (localGame) {
						...
						if (currentMove > 0){
							...
							// Gameboard update ****************
							Gameboard.getInstance().updateGameboard();
							//Gameboard.getInstance().switchPlayer();
							if (player1Turn == true){ // player is black
								printValidMoves(black);
								Gameboard.getInstance().setPlayer(false);
							} else {
								printValidMoves(white);
								Gameboard.getInstance().setPlayer(true);
							}
							updateScore();
							Gameboard.getInstance().updateCurrentScore(whiteCount, blackCount);
							// *********************************
						}
						else {
						...
						if (currentMove > 1){
							playerUndo = true;
							currentMove -= 2; // go back two moves and keep the same player
							printBoard();
							
							// Gameboard update ****************
							Gameboard.getInstance().updateGameboard();
							if (Gameboard.getInstance().getHumanPlayer() == true) {// human is black
								if (player1Turn == true){
									printValidMoves(black);
									Gameboard.getInstance().setPlayer(false);
								} else {
				...
				public static void redo(){
					if (localGame) {
						if (currentMove+1 <= furthestRedo){
							...
							// Gameboard update ****************
							Gameboard.getInstance().updateGameboard();
							if (player1Turn == true){ // if player is black 
								printValidMoves(black);
								Gameboard.getInstance().setPlayer(false);
							} else {
								printValidMoves(white);
						...
						if (currentMove+2 <= furthestRedo){
							currentMove += 2;
							printBoard(); //progress forward two moves and keep the same player
				...
				public static void performMove(int column, int row, Piece player){
					playerUndo = false;
					furthestRedo = currentMove;
					copyState();
					move(column, row, player);
				...
				public static void move(int column, int row, Piece player) {
					// if valid move...
					//update Gameboard ***************
					Gameboard.getInstance().clearValidPieces();
					// *******************************
					if (isValidMove(column, row, player, false)) {
						...
						//update Gameboard ***************
						Gameboard.getInstance().madeValidMove(true);
						if (player == white){
							Gameboard.getInstance().addWhitePiece(column, row);
							Gameboard.getInstance().setPlayer(false); // changes to black
							printValidMoves(black);
						} else {
							System.out.println("player is black");
							Gameboard.getInstance().addBlackPiece(column, row);
							Gameboard.getInstance().setPlayer(true); // changes to white
							printValidMoves(white);
						}
						// *******************************
						...
						// update Gameboard Score ********
						Gameboard.getInstance().updateCurrentScore(whiteCount, blackCount);
						checkGameStatus(player.getOpposite());
						// *******************************
					} else {
				...
				public static void flipPieces(int startCol, int endCol, int startRow, int endRow) {
					...
					if (startCol == endCol) {
						...
						for(int i=(startRow+1); i<endRow; i++) {
							...
							if (numPiecesFlippedWasRun == false){
								Gameboard.getInstance().flipPieceAnimation(startCol, i);
							}
							// *******************************
						}
						...
						for(int i=(startCol+1); i<endCol; i++) {
							...
							//update the gameboard ***********
							if (numPiecesFlippedWasRun == false){
								Gameboard.getInstance().flipPieceAnimation(i, startRow);
							}
							// *******************************
						}
					} else {
						...
						if (colAsc && rowAsc) {
							...
							for(int i=(startCol+1); i<(endCol); i++, j++) {
								...
								//update the gameboard ***********
								if (numPiecesFlippedWasRun == false){
									Gameboard.getInstance().flipPieceAnimation(i, j);
								}
								// *******************************
							}
						...
						for(int j=0; j<8; j++){
							if (isValidMove(i,j,player, true)){ //current player has a valid move;
								if (player == Piece.WHITE){
									Gameboard.getInstance().updateStatusText("> WHITE'S TURN...");
								} else {
									Gameboard.getInstance().updateStatusText("> BLACK'S TURN...");
								}
								gameOver = false;
								return;
						...
						for(int l=0; l<8; l++){
							if (isValidMove(k,l,player, true)){ //next player has a valid move;
								...
								if (player.getOpposite() == Piece.WHITE){ 
									...
									// *******************************
									printValidMoves(Piece.BLACK);
									Gameboard.getInstance().updateStatusText("> WHITE LOSES A TURN..");
									Gameboard.getInstance().setPlayer(false);
									...
									// *******************************
									printValidMoves(Piece.WHITE);
									Gameboard.getInstance().updateStatusText("> BLACK LOSES A TURN..");
									Gameboard.getInstance().setPlayer(true);
									new Handler().postDelayed(new Runnable()
								    {
								        public void run()
				...
				public static void checkWinner() {
					...
					if (whiteCount > blackCount) {
						...
						// ******************
						if (Gameboard.getInstance().getHumanPlayer() == false) {
							updateScoresOnFile(true);
						} else {
							updateScoresOnFile(false);
						}
						message2 = "> WHITE WINS!";
						// ******************
						...
						// ******************
						if (Gameboard.getInstance().getHumanPlayer() == true) {
							updateScoresOnFile(true);
						} else {
							updateScoresOnFile(false);
						}
					...
					// update Gameboard *************
					Gameboard.getInstance().gameOver(message2);
					writeScoresFile();
					// ******************************
				}
				
				public static void updateScoresOnFile(boolean win) {

					if (GameChoice.getChoiceInstance().playHuman) { return; } // local game
					
					// open the file
					openScoresFile();
					
					
					boolean easy = GameChoice.getChoiceInstance().playEasy;
					boolean medium = GameChoice.getChoiceInstance().playMedium;
					boolean hard = GameChoice.getChoiceInstance().playHard;
					boolean random = GameChoice.getChoiceInstance().playRandom;
				...
				public static void openScoresFile() {
					try {
						...
						String easyLosses = "", mediumLosses = "", hardLosses = "", randomLosses = "";
						...
						int[] theseScores = {Integer.parseInt(easyWins),
								Integer.parseInt(mediumWins),
								Integer.parseInt(hardWins),
								Integer.parseInt(randomWins),
								Integer.parseInt(easyLosses),
								Integer.parseInt(mediumLosses),
								Integer.parseInt(hardLosses),
								Integer.parseInt(randomLosses)
						};
				...
				public static void writeScoresFile() {
					try {
						File myFile = new File("/sdcard/thescores.xml");
						myFile.createNewFile();
				...
				public static int numPiecesFlipped(int col, int row) {
					...
					// Gameboard help function *********
					numPiecesFlippedWasRun = true;
					// *********************************
					
					move(col, row, player2);
					
					// Gameboard help function *********
					numPiecesFlippedWasRun = false;
					// *********************************
